...
var taskCollections *mongo.Collection = database.OpenCollection(database.Client, "task")


func GetTasksDb() ([]*models.Task, error) {
    cur, err := taskCollections.Find(context.TODO(), bson.D{{}})
    if err != nil {
...
func GetTasksByIdDb(id string) (*models.Task, error) {
    ...
    err := taskCollections.FindOne(context.TODO(), filter).Decode(&result)
    if err != nil {
...
func DeleteByIdDb(id string) (int64, error) {
    ...
    result, err := taskCollections.DeleteMany(context.TODO(), bson.D{{Key: "id", Value: id}})
    if err != nil {
...
func UpdateTaskDb(id string, updatedTask models.Task) error {
    ...
    if len(update) > 0 {
        ...
        _, err := taskCollections.UpdateOne(context.TODO(), filter, update)
        if err != nil {
...
func AddTaskDb(newTask models.Task) error {
    ...
    _, err := taskCollections.InsertOne(context.TODO(), task)
    if err != nil {
        return err // Return the error to be handled by the controller
    }
