...
import (
	...
	"github.com/bisratberhanu/A2SV-GO-Learning-Path/go_jwt_project/database"
	"github.com/bisratberhanu/A2SV-GO-Learning-Path/go_jwt_project/helpers"
	"github.com/bisratberhanu/A2SV-GO-Learning-Path/go_jwt_project/models"
...
func VerifyPassword(userPassword string, providedPassword string)(bool, string){
	...
	if err!=nil{
		check=false
		fmt.Sprintf("password or email not found")
	}
	return check, msg
}
...
func SignUp() gin.HandlerFunc {
	...
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		...
		if err!=nil{
			log.Panic(err)
			c.JSON(http.StatusBadRequest, gin.H{"error":"error occured while getting email"})
		}
		count, err = userCollections.CountDocuments(ctx, bson.M{"phone": user.Phone})
		defer cancel()
		if err!=nil{
			log.Panic(err)
			c.JSON(http.StatusBadRequest, gin.H{"error":"error occured while getting phone"})
			return 
		}
		if count > 0{
			c.JSON(http.StatusBadRequest, gin.H{"error": "email or phonenumber already exists"})
			return 
		}
		...
		defer cancel()
		c.JSON(http.StatusOK,gin.H{"error" : resultInsertionNumber})
